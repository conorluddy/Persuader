...
import type { ProviderAdapter, ProviderError } from './provider.js';
...
export interface PipelineStage<TInput = unknown, TOutput = unknown> {
  ...
  readonly transformInput?: (input: TInput, context: PipelineExecutionContext) => unknown | Promise<unknown>;
  ...
  readonly condition?: (input: TInput, context: PipelineExecutionContext) => boolean | Promise<boolean>;
...
export interface StageRetryConfig {
  ...
  readonly strategy?: 'exponential_backoff' | 'fixed_delay' | 'immediate' | 'custom';
 ...
 */
export interface Pipeline<TInput = unknown, TFinalOutput = unknown> {
...
export interface PipelineSessionConfig {
  ...
  readonly contextStrategy?: 'accumulate' | 'sliding_window' | 'stage_isolated' | 'smart_context';
...
export interface ProgressConfig {
  ...
  readonly metricsToTrack?: ReadonlyArray<'tokens' | 'cost' | 'timing' | 'success_rate' | 'quality' | 'confidence'>;
...
export interface ReviewInterfaceConfig {
  ...
  readonly cli?: {
    ...
    readonly autoExpand?: ReadonlyArray<'input' | 'output' | 'metadata' | 'errors' | 'history'>;
...
export interface PipelineErrorHandling {
  ...
  readonly strategy: 'fail_fast' | 'continue_on_error' | 'collect_errors' | 'adaptive';
...
export interface PipelineExecutionState {
  ...
  readonly phase: 'initializing' | 'executing' | 'reviewing' | 'completing' | 'failed' | 'cancelled';
...
export interface ReviewManager {
  requestReview(stage: PipelineStage, result: StageResult, context: PipelineExecutionContext): Promise<ReviewResult>;
  isReviewRequired(stage: PipelineStage, result: StageResult, context: PipelineExecutionContext): Promise<boolean>;
}
